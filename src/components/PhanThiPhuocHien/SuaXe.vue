<template>
    <div class="p-margin-desktop">
        <div class="flex items-center gap-sm mb-lg">
            ...
            <h2 class="font-headline-lg text-headline-lg text-on-surface">Cập nhật phương tiện</h2>
        </div>
    ...
    methods: {
        ...
        async fetchXe() {
            try {
                ...
                const response = await api.get(`/xe-may/${id}`);
                this.form = {
                    ten_xe: response.ten_xe,
                    bien_so_xe: response.bien_so_xe,
                    mau_sac: response.mau_sac,
                    gia_thue_ngay: response.gia_thue_ngay,
                    ma_danh_muc: response.ma_danh_muc,
                    hinh_anh: response.hinh_anh
                };
                this.isReady = response.tinh_trang_xe === 'SanSang';
            } catch (error) {
                console.error("Failed to fetch vehicle details", error);
                alert("Không tìm thấy dữ liệu xe máy");
        ...
        async submitForm() {
            ...
            try {
                ...
                await api.put(`/admin/xe-may/${id}`, payload);
                alert("Cập nhật xe thành công!");
                this.$router.push('/admin/quan-ly-xe');
            } catch (error) {
                alert("Lỗi khi cập nhật xe: " + (error.message || 'Check console'));
                console.error(error);
            } finally {
                this.saving = false;
            }
